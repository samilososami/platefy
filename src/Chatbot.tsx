...
import { ArrowLeft, ArrowRight, ArrowUpRight, AudioLines, BookOpen, Brain, Check, Clock3, Cpu, Info, KeyRound, Leaf, Menu, Mic, Plus, RotateCcw, Square, Store, Users, Volume2, VolumeX, X } from 'lucide-react';
import type { Locale } from './content';
import { Brand, LanguageSelect, Orb, type OrbState } from './ui';
import { AI_PROVIDERS, MODEL_INFO, generateReply, getInferenceMetrics, getModelStatus, prepareKnowledge, providerErrorMessage, subscribeInferenceMetrics, subscribeModelStatus, synthesizeSpeech, ProviderError, type ChatMessage, type InferenceMetrics, type ModelStatus } from './services/ai';
import type { MenuDish } from './services/restaurant';
import './chatbot.css';
...
const copy = {
  es: {
    ...
    disclaimer: 'IA en la nube · Carta de muestra · Las reservas no se confirman.', details: 'Detalles', detailsTitle: 'Una conversación con la carta.', detailsBody: 'Esta experiencia carga MENU.json y PL8.md. El servidor filtra la carta y Qwen3 redacta la respuesta.', provider: 'Inteligencia y voz', providerBody: 'Qwen3 30B-A3B responde mediante Cloudflare Workers AI. El servidor selecciona primero los platos compatibles y nunca acepta un menú ni instrucciones del navegador. Si activas la voz, la respuesta se envía además al servicio Kokoro.', privacy: 'Los últimos mensajes y solo los fragmentos necesarios de la carta se envían a Cloudflare Workers AI. Evita compartir datos personales.', retry: 'Volver a intentar', dismiss: 'Cerrar aviso', micUnsupported: 'Este navegador no ofrece reconocimiento de voz. Puedes seguir escribiendo o probar con Chrome.', micDenied: 'No se ha permitido el micrófono. Puedes habilitarlo en tu navegador o seguir escribiendo.', micError: 'No he podido escuchar tu voz. Puedes volver a intentarlo o escribir tu pregunta.', micNoSpeech: 'No he oído ninguna voz. Puedes intentarlo de nuevo.', audioError: 'El audio no se ha podido reproducir. Pulsa Escuchar respuesta para volver a intentarlo.', thinkingMode: 'Razonar más', thinkingUnavailable: 'Qwen3 responde sin razonamiento extendido en modo rápido', modelFirstLoad: 'La carta se verifica antes de cada respuesta.', metrics: 'Rendimiento de Workers AI', apiSettings: 'Estado de API',
    menuIntro: 'Treinta y cuatro referencias para probar recomendaciones reales.', vegan: 'Vegano', vegetarian: 'Vegetariano', milk: 'Alérgenos', allergy: 'Las recetas son de muestra. La ausencia de alérgenos y las trazas deben confirmarse con el equipo de un restaurante real.', hoursIntro: 'Una mesa mediterránea para imaginar tu próxima visita.', lunch: 'Comidas', dinner: 'Cenas', location: 'Sobre este local', locationBody: 'Este restaurante es ficticio. No hay una dirección ni un teléfono reales asociados.', reservations: 'Reservas', reservationsBody: 'Puedes preguntar cómo sería una reserva, pero esta experiencia no consulta disponibilidad ni confirma mesas.', you: 'Tú', voiceHint: 'Voz natural en español e inglés',
  },
  en: {
    ...
    disclaimer: 'Cloud AI · Sample menu · Reservations are not confirmed.', details: 'Details', detailsTitle: 'A conversation grounded in the menu.', detailsBody: 'This experience loads MENU.json and PL8.md. The server filters the menu and Qwen3 writes the reply.', provider: 'Intelligence & voice', providerBody: 'Qwen3 30B-A3B responds through Cloudflare Workers AI. The server first selects compatible dishes and never accepts menu data or instructions from the browser. If sound is enabled, the reply is also sent to Kokoro.', privacy: 'Recent messages and only the required menu fragments are sent to Cloudflare Workers AI. Avoid sharing personal data.', retry: 'Try again', dismiss: 'Dismiss notice', micUnsupported: 'This browser does not offer voice recognition. You can keep typing or try Chrome.', micDenied: 'Microphone access was not allowed. You can enable it in your browser or keep typing.', micError: 'I could not hear your voice. You can try again or type your question.', micNoSpeech: 'I did not hear any speech. You can try again.', audioError: 'The audio could not play. Select Listen to reply to try again.', thinkingMode: 'Reason more', thinkingUnavailable: 'Qwen3 answers without extended reasoning in fast mode', modelFirstLoad: 'The menu is verified before every reply.', metrics: 'Workers AI performance', apiSettings: 'API status',
    menuIntro: 'Thirty-four items for testing grounded recommendations.', vegan: 'Vegan', vegetarian: 'Vegetarian', milk: 'Allergens', allergy: 'These are sample recipes. Allergens and cross-contamination must be checked with the team at a real restaurant.', hoursIntro: 'A Mediterranean table to imagine your next visit.', lunch: 'Lunch', dinner: 'Dinner', location: 'About this restaurant', locationBody: 'This restaurant is fictional. It has no real address or phone number.', reservations: 'Reservations', reservationsBody: 'You can ask how a booking would work, but this experience does not check availability or confirm tables.', you: 'You', voiceHint: 'Natural voice in Spanish and English',
  },
  ca: {
    ...
    disclaimer: 'IA al núvol · Carta de mostra · Les reserves no es confirmen.', details: 'Detalls', detailsTitle: 'Una conversa basada en la carta.', detailsBody: 'Aquesta experiència carrega MENU.json i PL8.md. El servidor filtra la carta i Qwen3 redacta la resposta.', provider: 'Intel·ligència i veu', providerBody: 'Qwen3 30B-A3B respon mitjançant Cloudflare Workers AI. El servidor selecciona primer els plats compatibles i no accepta cap carta ni instruccions del navegador. Si actives el so, la resposta també s’envia a Kokoro.', privacy: 'Els últims missatges i només els fragments necessaris de la carta s’envien a Cloudflare Workers AI. Evita compartir dades personals.', retry: 'Tornar-ho a provar', dismiss: 'Tancar avís', micUnsupported: 'Aquest navegador no ofereix reconeixement de veu. Pots continuar escrivint o provar Chrome.', micDenied: 'No s’ha permès el micròfon. Pots habilitar-lo al navegador o continuar escrivint.', micError: 'No he pogut escoltar la teva veu. Pots tornar-ho a provar o escriure la pregunta.', micNoSpeech: 'No he sentit cap veu. Pots tornar-ho a provar.', audioError: 'No s’ha pogut reproduir l’àudio. Prem Escoltar resposta per tornar-ho a provar.', thinkingMode: 'Raonar més', thinkingUnavailable: 'Qwen3 respon sense raonament ampliat en mode ràpid', modelFirstLoad: 'La carta es verifica abans de cada resposta.', metrics: 'Rendiment de Workers AI', apiSettings: 'Estat de l’API',
    menuIntro: 'Trenta-quatre referències per provar recomanacions fonamentades.', vegan: 'Vegà', vegetarian: 'Vegetarià', milk: 'Al·lèrgens', allergy: 'Les receptes són de mostra. Cal confirmar els al·lèrgens i les traces amb l’equip d’un restaurant real.', hoursIntro: 'Una taula mediterrània per imaginar la pròxima visita.', lunch: 'Dinars', dinner: 'Sopars', location: 'Sobre aquest local', locationBody: 'Aquest restaurant és fictici. No té cap adreça ni telèfon reals associats.', reservations: 'Reserves', reservationsBody: 'Pots preguntar com seria una reserva, però aquesta experiència no consulta disponibilitat ni confirma taules.', you: 'Tu', voiceHint: 'Veu natural en castellà i anglès',
  },
} as const;
...
function useConversation(locale: Locale, thinking: boolean) {
  ...
  const stopGeneration = useCallback(() => {
    ...
    if (liveRef.current) { setGenerating(false); setPartialReply(''); }
  }, []);
  ...
  const requestReply = useCallback(async (history: DisplayMessage[]) => {
    ...
    retryRef.current = history; setPartialReply(''); setGenerating(true);
    try {
      const answer = await generateReply(history.map(({ role, content }) => ({ role, content })), locale, controller.signal, text => {
        if (!controller.signal.aborted && liveRef.current) setPartialReply(text);
      }, thinking);
      if (controller.signal.aborted || !liveRef.current) return;
      const reply = makeMessage('assistant', answer);
      const next = [...history, reply]; messagesRef.current = next; setMessages(next);
      generationRef.current = null; setPartialReply(''); setGenerating(false);
  ...
  }, [locale, playMessage, stopAudio, stopGeneration, thinking]);
  ...
  const reset = useCallback(() => {
    ...
    setMessages([]); setPartialReply(''); setNotice(null); setCanRetry(false);
  }, [stopAudio, stopGeneration]);
  ...
  return { messages, partialReply, generating, audioState, activeAudio, sound, notice, canRetry, send, reset, stopGeneration, stopAudio, playMessage, changeLanguage, toggleSound, setNotice: showNotice, dismiss: () => showNotice(null), retry: () => void requestReply(retryRef.current) };
}
...
function Sidebar({ c, onPanel, onReset, home }: { c: Copy; onPanel: (panel: Panel) => void; onReset: () => void; home: string }) {
  return <>
    ...
    <div className="chat-rail-bottom"><a href="/api/"><KeyRound size={20} aria-hidden="true" />{c.apiSettings}</a><button onClick={() => onPanel('details')}><Store size={20} aria-hidden="true" />{c.sample}</button><a href={home}><ArrowLeft size={20} aria-hidden="true" />{c.back}</a></div>
  </>;
}

function InformationPanel({ panel, onClose, onPanel, onReset, locale, home, menu, metrics }: { panel: Exclude<Panel, null>; onClose: () => void; onPanel: (panel: Panel) => void; onReset: () => void; locale: Locale; home: string; menu: MenuDish[]; metrics: InferenceMetrics | null }) {
  ...
  return <dialog ref={ref} className={`chat-dialog chat-dialog--${panel}`} aria-labelledby="chat-panel-title" onCancel={onClose} onClick={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="chat-dialog-inner">
      ...
      {panel === 'details' ? <><p className="chat-dialog-intro">{c.detailsBody}</p><section className="chat-info-section"><h3>{c.provider}</h3><p>{c.providerBody}</p><div className="chat-provider-links"><a href={AI_PROVIDERS.chat.space} target="_blank" rel="noreferrer">{AI_PROVIDERS.chat.name}<ArrowUpRight size={15} /></a><a href="/api/">{c.apiSettings}<ArrowUpRight size={15} /></a><a href="../menu/MENU.json" target="_blank">MENU.json<ArrowUpRight size={15} /></a><a href="../menu/PL8.md" target="_blank">PL8.md<ArrowUpRight size={15} /></a><a href={AI_PROVIDERS.speech.space} target="_blank" rel="noreferrer">Kokoro · ES<ArrowUpRight size={15} /></a><a href={AI_PROVIDERS.speechEnglish.space} target="_blank" rel="noreferrer">Kokoro · EN<ArrowUpRight size={15} /></a></div><p>{c.voiceHint}</p></section>{metrics ? <section className="chat-metrics"><h3><Cpu size={16} />{c.metrics}</h3><dl><div><dt>Modelo</dt><dd>Qwen3 30B-A3B</dd></div><div><dt>Proveedor</dt><dd>{metrics.provider}</dd></div><div><dt>Respuesta</dt><dd>{(metrics.responseMs / 1000).toFixed(2)} s</dd></div><div><dt>Primer token</dt><dd>{metrics.firstTokenMs === null ? '—' : `${(metrics.firstTokenMs / 1000).toFixed(2)} s`}</dd></div><div><dt>Velocidad</dt><dd>{metrics.tokensPerSecond === null ? '—' : `${metrics.tokensPerSecond.toFixed(1)} tok/s`}</dd></div><div><dt>Razonamiento</dt><dd>{metrics.reasoning}</dd></div><div><dt>Entrada</dt><dd>{metrics.promptTokens ?? '—'} tokens</dd></div><div><dt>Salida</dt><dd>{metrics.completionTokens ?? '—'} tokens</dd></div><div><dt>Consumo</dt><dd>{metrics.neurons === null ? '—' : `${metrics.neurons.toFixed(2)} neurons`}</dd></div></dl></section> : null}<p className="chat-panel-note"><Info size={17} aria-hidden="true" />{c.privacy}</p></> : null}
    </div>
  </dialog>;
}
...
export default function Chatbot({ locale, onLocaleChange }: { locale: Locale; onLocaleChange: (locale: Locale) => void }) {
  ...
  const conversation = useConversation(locale, thinking);
  ...
  const [metrics, setMetrics] = useState<InferenceMetrics | null>(() => getInferenceMetrics());
  ...
  useEffect(() => {
    const unsubscribeStatus = subscribeModelStatus(setModelStatus);
    const unsubscribeMetrics = subscribeInferenceMetrics(setMetrics);
    void prepareKnowledge().then(knowledge => setMenu(knowledge.menu.platos)).catch(() => undefined);
    return () => { unsubscribeStatus(); unsubscribeMetrics(); };
  }, []);
  useEffect(() => {
  ...
  return <div ref={appRef} className={`chat-app ${started ? 'chat-app--started' : 'chat-app--welcome'}`}>
    ...
    <main className="chat-main" aria-label={c.table}>
      <header className="chat-header"><div className="chat-header-title"><button className="icon-button chat-menu-toggle" onClick={() => openPanel('navigation')} aria-label={c.openNavigation}><Menu size={21} /></button><div><h1>{c.table}</h1><p>{c.sample}</p></div></div><div className="chat-header-actions"><button className="chat-thinking-toggle" onClick={() => setThinking(value => !value)} disabled={!MODEL_INFO.thinking || conversation.generating} aria-pressed={thinking} title={thinking ? c.thinkingMode : c.thinkingUnavailable}><Brain size={17} /><span>{c.thinkingMode}</span></button><button className="chat-sound" onClick={conversation.toggleSound} aria-label={conversation.sound ? c.soundOff : c.soundOn} aria-pressed={conversation.sound} title={c.voiceHint}>{conversation.sound ? <Volume2 size={18} /> : <VolumeX size={18} />}<span>{c.sound}</span><i aria-hidden="true" className={conversation.sound ? 'is-on' : ''} /></button><LanguageSelect locale={locale} onChange={changeLocale} /></div></header>
      ...
      {panel ? <InformationPanel key={panel} panel={panel} onClose={() => setPanel(null)} onPanel={openPanel} onReset={reset} locale={locale} home={home} menu={menu} metrics={metrics} /> : null}
    </main>
  </div>;
}
