...
const copy = {
  es: {
    table: 'La mesa de Platefy', sample: 'Restaurante de muestra', newChat: 'Nueva conversación', about: 'Sobre la mesa', menu: 'Carta del restaurante', hours: 'Horarios y local', back: 'Volver a la carta', sound: 'Sonido', soundOn: 'Activar respuestas por voz', soundOff: 'Desactivar respuestas por voz',
    ...
    disclaimer: 'Carta de muestra · Confirma los alérgenos con el restaurante.', details: 'Detalles', detailsTitle: 'Una conversación con la carta.', detailsBody: 'Soy platefy, tu compañía para descubrir la carta. Pregúntame por platos, ingredientes, precios o fotografías.', privacy: 'Tus preguntas se procesan mediante un servicio de inteligencia artificial. Si activas el sonido, el texto también se envía al servicio de voz. Evita compartir datos personales.', retry: 'Volver a intentar', dismiss: 'Cerrar aviso', micUnsupported: 'Este navegador no ofrece reconocimiento de voz. Puedes seguir escribiendo o probar con Chrome.', micDenied: 'No se ha permitido el micrófono. Puedes habilitarlo en tu navegador o seguir escribiendo.', micError: 'No he podido escuchar tu voz. Puedes volver a intentarlo o escribir tu pregunta.', micNoSpeech: 'No he oído ninguna voz. Puedes intentarlo de nuevo.', audioError: 'El audio no se ha podido reproducir. Pulsa Escuchar respuesta para volver a intentarlo.',
    menuIntro: 'Descubre los platos de esta carta.', vegan: 'Vegano', vegetarian: 'Vegetariano', milk: 'Alérgenos', allergy: 'Las recetas son de muestra. La ausencia de alérgenos y las trazas deben confirmarse con el equipo de un restaurante real.', hoursIntro: 'Los horarios de este restaurante de muestra no están publicados.', lunch: 'Comidas', dinner: 'Cenas', location: 'Sobre este local', locationBody: 'Este restaurante es ficticio. No hay una dirección ni un teléfono reales asociados.', reservations: 'Reservas', reservationsBody: 'Puedes preguntar cómo sería una reserva, pero esta experiencia no consulta disponibilidad ni confirma mesas.', you: 'Tú', voiceHint: 'Voz natural en español e inglés',
  },
  en: {
    table: 'The Platefy table', sample: 'Sample restaurant', newChat: 'New conversation', about: 'Around the table', menu: 'Restaurant menu', hours: 'Hours & restaurant', back: 'Back to the menu', sound: 'Sound', soundOn: 'Enable spoken replies', soundOff: 'Disable spoken replies',
    ...
    disclaimer: 'Sample menu · Check allergens with the restaurant.', details: 'Details', detailsTitle: 'A conversation grounded in the menu.', detailsBody: 'I’m platefy, here to help you explore the menu. Ask about dishes, ingredients, prices or photos.', privacy: 'Your questions are processed by an AI service. If sound is enabled, the text is also sent to a voice service. Avoid sharing personal information.', retry: 'Try again', dismiss: 'Dismiss notice', micUnsupported: 'This browser does not offer voice recognition. You can keep typing or try Chrome.', micDenied: 'Microphone access was not allowed. You can enable it in your browser or keep typing.', micError: 'I could not hear your voice. You can try again or type your question.', micNoSpeech: 'I did not hear any speech. You can try again.', audioError: 'The audio could not play. Select Listen to reply to try again.',
    menuIntro: 'Explore the dishes on this menu.', vegan: 'Vegan', vegetarian: 'Vegetarian', milk: 'Allergens', allergy: 'These are sample recipes. Allergens and cross-contamination must be checked with the team at a real restaurant.', hoursIntro: 'Opening hours for this sample restaurant have not been published.', lunch: 'Lunch', dinner: 'Dinner', location: 'About this restaurant', locationBody: 'This restaurant is fictional. It has no real address or phone number.', reservations: 'Reservations', reservationsBody: 'You can ask how a booking would work, but this experience does not check availability or confirm tables.', you: 'You', voiceHint: 'Natural voice in Spanish and English',
  },
  ca: {
    table: 'La taula de Platefy', sample: 'Restaurant de mostra', newChat: 'Nova conversa', about: 'Sobre la taula', menu: 'Carta del restaurant', hours: 'Horaris i local', back: 'Tornar a la carta', sound: 'So', soundOn: 'Activar respostes amb veu', soundOff: 'Desactivar respostes amb veu',
...
function useConversation(locale: Locale) {
  ...
  const requestReply = useCallback(async (history: DisplayMessage[]) => {
    ...
    try {
      ...
      }, false, value => { images = value; });
      if (controller.signal.aborted || !liveRef.current) return;
      const reply = { ...makeMessage('assistant', answer), images };
...
function Sidebar({ c, onPanel, onReset, home }: { c: Copy; onPanel: (panel: Panel) => void; onReset: () => void; home: string }) {
  ...
  return <>
    ...
    <ChatbotGarden restaurant={restaurant} />
...
function MessageText({ text }: { text: string }) {
  ...
  let items: Array<{ ordered: boolean; text: string }> = [];
  const flushList = () => {
    if (!items.length) return;
    const ordered = items[0].ordered;
    const Tag = ordered ? 'ol' : 'ul';
    blocks.push(<Tag key={`list-${blocks.length}`}>{items.map((item, i) => <li key={i}><InlineText text={item.text} /></li>)}</Tag>);
    items = [];
  };
  text.trim().split(/\n/).forEach((line, index) => {
    const match = line.match(/^\s*(?:(\d+)[.)]|[-*•])\s+(.+)$/);
    if (match) { items.push({ ordered: Boolean(match[1]), text: match[2] }); return; }
    flushList();
    if (line.trim()) blocks.push(<p key={`p-${index}`}><InlineText text={line.trim()} /></p>);
  });
  flushList();
