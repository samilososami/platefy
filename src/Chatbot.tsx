...
import { ArrowLeft, ArrowRight, ArrowUpRight, AudioLines, BookOpen, Brain, Check, Clock3, Cpu, Info, Leaf, Menu, Mic, Plus, RotateCcw, Square, Store, Users, Volume2, VolumeX, X } from 'lucide-react';
...
const copy = {
  es: {
    ...
    disclaimer: 'IA local en tu navegador · Restaurante de muestra · Las reservas no se confirman.', details: 'Detalles', detailsTitle: 'Una conversación, en tu dispositivo.', detailsBody: 'Esta experiencia carga MENU.json y PL8.md. Qwen se ejecuta mediante WebLLM dentro de tu navegador.', provider: 'Inteligencia y voz', providerBody: 'Las preguntas y la carta se procesan localmente con Qwen2.5 1.5B. La primera consulta descarga cerca de 1,5 GB y el navegador puede conservarlos en caché. Si activas la voz, el texto de la respuesta sí se envía al servicio Kokoro.', privacy: 'El chat y el menú no se envían al modelo en la nube. Evita datos personales si activas la voz.', retry: 'Volver a intentar', dismiss: 'Cerrar aviso', micUnsupported: 'Este navegador no ofrece reconocimiento de voz. Puedes seguir escribiendo o probar con Chrome.', micDenied: 'No se ha permitido el micrófono. Puedes habilitarlo en tu navegador o seguir escribiendo.', micError: 'No he podido escuchar tu voz. Puedes volver a intentarlo o escribir tu pregunta.', micNoSpeech: 'No he oído ninguna voz. Puedes intentarlo de nuevo.', audioError: 'El audio no se ha podido reproducir. Pulsa Escuchar respuesta para volver a intentarlo.', thinkingMode: 'Pensamiento', thinkingUnavailable: 'Qwen2.5 no ofrece un modo de pensamiento nativo', modelFirstLoad: 'La primera respuesta descargará el modelo en este dispositivo.', metrics: 'Rendimiento local',
  ...
  en: {
    ...
    disclaimer: 'On-device AI · Sample restaurant · Reservations are not confirmed.', details: 'Details', detailsTitle: 'A conversation, on your device.', detailsBody: 'This experience loads MENU.json and PL8.md. Qwen runs through WebLLM inside your browser.', provider: 'Intelligence & voice', providerBody: 'Questions and menu data are processed locally with Qwen2.5 1.5B. The first query downloads about 1.5 GB, which the browser may cache. If you enable sound, reply text is sent to the Kokoro voice service.', privacy: 'Chat and menu data are not sent to a cloud model. Avoid personal data if you enable voice.', retry: 'Try again', dismiss: 'Dismiss notice', micUnsupported: 'This browser does not offer voice recognition. You can keep typing or try Chrome.', micDenied: 'Microphone access was not allowed. You can enable it in your browser or keep typing.', micError: 'I could not hear your voice. You can try again or type your question.', micNoSpeech: 'I did not hear any speech. You can try again.', audioError: 'The audio could not play. Select Listen to reply to try again.', thinkingMode: 'Thinking', thinkingUnavailable: 'Qwen2.5 has no native thinking mode', modelFirstLoad: 'The first reply will download the model to this device.', metrics: 'Local performance',
  ...
  ca: {
    ...
    disclaimer: 'IA local al navegador · Restaurant de mostra · Les reserves no es confirmen.', details: 'Detalls', detailsTitle: 'Una conversa, al teu dispositiu.', detailsBody: 'Aquesta experiència carrega MENU.json i PL8.md. Qwen s’executa amb WebLLM dins del navegador.', provider: 'Intel·ligència i veu', providerBody: 'Les preguntes i la carta es processen localment amb Qwen2.5 1.5B. La primera consulta baixa prop d’1,5 GB que el navegador pot desar en memòria cau. Si actives el so, el text sí que s’envia al servei de veu Kokoro.', privacy: 'El xat i la carta no s’envien a un model al núvol. Evita dades personals si actives la veu.', retry: 'Tornar-ho a provar', dismiss: 'Tancar avís', micUnsupported: 'Aquest navegador no ofereix reconeixement de veu. Pots continuar escrivint o provar Chrome.', micDenied: 'No s’ha permès el micròfon. Pots habilitar-lo al navegador o continuar escrivint.', micError: 'No he pogut escoltar la teva veu. Pots tornar-ho a provar o escriure la pregunta.', micNoSpeech: 'No he sentit cap veu. Pots tornar-ho a provar.', audioError: 'No s’ha pogut reproduir l’àudio. Prem Escoltar resposta per tornar-ho a provar.', thinkingMode: 'Pensament', thinkingUnavailable: 'Qwen2.5 no ofereix un mode de pensament natiu', modelFirstLoad: 'La primera resposta baixarà el model al dispositiu.', metrics: 'Rendiment local',
...
function InformationPanel({ panel, onClose, onPanel, onReset, locale, home, menu, metrics }: { panel: Exclude<Panel, null>; onClose: () => void; onPanel: (panel: Panel) => void; onReset: () => void; locale: Locale; home: string; menu: MenuDish[]; metrics: InferenceMetrics | null }) {
  ...
  return <dialog ref={ref} className={`chat-dialog chat-dialog--${panel}`} aria-labelledby="chat-panel-title" onCancel={onClose} onClick={event => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="chat-dialog-inner">
      ...
      {panel === 'details' ? <><p className="chat-dialog-intro">{c.detailsBody}</p><section className="chat-info-section"><h3>{c.provider}</h3><p>{c.providerBody}</p><div className="chat-provider-links"><a href={AI_PROVIDERS.chat.space} target="_blank" rel="noreferrer">{AI_PROVIDERS.chat.name}<ArrowUpRight size={15} /></a><a href="../menu/MENU.json" target="_blank">MENU.json<ArrowUpRight size={15} /></a><a href="../menu/PL8.md" target="_blank">PL8.md<ArrowUpRight size={15} /></a><a href={AI_PROVIDERS.speech.space} target="_blank" rel="noreferrer">Kokoro · ES<ArrowUpRight size={15} /></a><a href={AI_PROVIDERS.speechEnglish.space} target="_blank" rel="noreferrer">Kokoro · EN<ArrowUpRight size={15} /></a></div><p>{c.voiceHint}</p></section>{metrics ? <section className="chat-metrics"><h3><Cpu size={16} />{c.metrics}</h3><dl><div><dt>Modelo</dt><dd>Qwen2.5 1.5B Q4</dd></div><div><dt>Respuesta</dt><dd>{(metrics.responseMs / 1000).toFixed(2)} s</dd></div><div><dt>Primer token</dt><dd>{metrics.firstTokenMs === null ? '—' : `${(metrics.firstTokenMs / 1000).toFixed(2)} s`}</dd></div><div><dt>Velocidad</dt><dd>{metrics.tokensPerSecond === null ? '—' : `${metrics.tokensPerSecond.toFixed(1)} tok/s`}</dd></div><div><dt>VRAM estimada</dt><dd>{Math.round(metrics.estimatedVramMB)} MB</dd></div><div><dt>JS heap Δ</dt><dd>{metrics.jsHeapDeltaMB === null ? 'No disponible' : `${metrics.jsHeapDeltaMB.toFixed(1)} MB`}</dd></div></dl></section> : null}<p className="chat-panel-note"><Info size={17} aria-hidden="true" />{c.privacy}</p></> : null}
    </div>
  </dialog>;
}
    ...
    <main className="chat-main" aria-label={c.table}>
      ...
      {!started ? <section className="chat-welcome"><Orb state={state} className="chat-welcome-orb" /><div className="chat-welcome-copy"><h2>{c.welcome}</h2><p>{c.welcomeBody}</p><span>{c.welcomeNote}</span></div><button className="button button-dark chat-start" onClick={() => { conversation.stopAudio(); setStarted(true); }}>{c.start}<ArrowRight size={18} aria-hidden="true" /></button><button type="button" className={`chat-voice-preview ${welcomeAudioActive ? 'chat-voice-preview--active' : ''}`} onClick={() => welcomeAudioActive ? conversation.stopAudio() : void conversation.playMessage(welcomeMessage)} aria-label={welcomeAudioActive ? c.stopAudio : c.voicePreview}>{welcomeAudioActive ? <Square size={12} fill="currentColor" aria-hidden="true" /> : <Volume2 size={15} aria-hidden="true" />}<span>{welcomeAudioActive ? conversation.audioState === 'loading' ? c.loadingAudio : c.stopAudio : c.voicePreview}</span></button><p className="chat-welcome-note">{c.sample}</p>{conversation.notice ? <p className="chat-welcome-notice" role="alert">{conversation.notice}</p> : null}</section> : <>
        ...
        <div className="chat-bottom">
          {modelStatus.phase === 'loading-model' ? <div className="chat-model-progress" role="status"><div><Cpu size={15} /><span>{modelStatus.text}</span><strong>{Math.round(modelStatus.progress * 100)}%</strong></div><progress max="1" value={modelStatus.progress} /></div> : null}
